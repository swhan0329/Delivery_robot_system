...
class Detection:
    '''address, stop detection class'''

    def __init__(self):
        self.flag_stop = False
        self.mode = 0 # 0: only green, 1: see address

        self.stop_time = 1.0
        self.stop_102_time = 1.5
        self.stop_202_time = 1.8
        self.enforce_102_stop_time = 2.5
        self.enforce_202_stop_time = 2.0
        self.enforce_time = 0.4
        self.status_detect_time = 0
        self.previous_status_detect_time = 0
        self.sequenctial_update_time = 1.1
    ...
    def find_address(self, image, mask):
        ...
        if contour:
            for cnt in contour:
                ...
                #cv2.imshow('contour', image)  # 이미지 출력
                #cv2.waitKey(0)
                #print("area:", area)
                #print("epsilon:", epsilon)
                #print("sizeapp:", sizeapp)
                if area > 800 and area < 2800 and epsilon < 7 and epsilon > 3:
                    if sizeapp is not 4:
                        pass
                    else:
                        displayCnt = approx
                        ...
                        if cropped.shape[0] == 0 or cropped.shape[1] == 0:
                            ...
                            if self.mode:
                                ...
                                thresh = cv2.threshold(gray, 0, 255,
                                                    cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
                                image = self.adjust_gamma(thresh, gamma=0.5)
                                image = cv2.erode(image, (7, 7), iterations=2)
                                image = cv2.dilate(image, (5, 5), iterations=3)
                                ROI_image = cv2.bitwise_not(image)
                                # cv2.imshow("ROI_image_cropped", ROI_image)
                                # cv2.waitKey(0)
                                ...
                                if not digitCnts:
                                    ...
                                    for c in digitCnts:
                                        ...
                                        for (i, ((xA, yA), (xB, yB))) in enumerate(segments):
                                            ...
                                            # if segROI.shape[0]>0 and segROI.shape[1]>0:
                                        ...
                                        if middle_line > 25:
                                            if self.wise == 0:
                                                if self.address == "102" and self.status == 1:
                                                    ...
                                                    self.status = 4
                                                    self.enforce_flag = True
                                                elif self.address == "101":
                                                    self.status = 0
                                                elif self.address == "103":
                                                    self.status = 2
                                                elif self.address == "201":
                                                    self.status = 5
                                                elif self.address == "203":
                                                    self.status = 3
                                            elif self.wise == 1:
                                                if self.address == "102" and self.status == 3:
                                                    ...
                                                    self.status = 5
                                            #print("enforce status change", self.status)
                            else:
                                pass
        # print("#############see green flag#############", self.see_green_flag)
        # print("#####QUEUE DATA#####",self.queue_data)
        if self.mode:
    ...
    def find_object(self, image, curr_orientation, mode):
        self.mode = mode

        if curr_orientation == 'c':
            self.wise = 1 #clock-wise
        else:
            self.wise = 0 #cclock-wise
        self.address = None
    ...
    def detect_flag(self, image, green_stop=20):
        if self.flag_stop:
            self.previous_status_detect_time = time.time()
            self.flag_stop = False
        ...
        if self.detect_stop_flag:
            ...
            if (time.time() - self.stop_time_start) > self.stop_time: #1.5
                if self.wise == 0:
            ...
            if self.wise == 0:
                if self.status == 6:  # 201 지나면서 count 시작
                    ...
                    if (time.time() - self.stop_time_start) > self.stop_202_time: #1.5
                        print("enforce stop")
                ...
                if self.status == 1:  # 101 지나면서 count 시작
                    if self.count_stop_frame_calc == 0:
                        self.stop_time_start = time.time()
                    self.count_stop_frame_calc += 1
                    if (time.time() - self.stop_time_start) > self.stop_102_time: #1.8
                        print("enforce stop")
                        self.status -= 1  # 102 status
                        self.status = self.status % 7
                        self.status_detect = True
        ...
        if self.enforce_mode:
            ...
            if (time.time() - self.enforcemode_time_start) > self.enforce_time:
                self.enforce_mode = False
                self.enforce_flag = False
                if self.wise == 0:
                    self.status += 1
            ...
            if len(self.Green_queue) > green_stop: #14
            ...
            if self.wise == 0:  # clock-wise
                ...
                if (self.status == 4 and not self.ready_flag):  # 203 지나면서 count 시작
                    if self.count_frame == 0:
                        self.force_update_time_start = time.time()
                    self.count_frame += 1
                if self.status == 1 and (time.time() - self.force_update_time_start > self.enforce_102_stop_time) and not self.ready_flag: #3.0
                    self.status += 1  # 102 status
                    self.status = self.status % 7
                    self.status_detect = True
                    self.count_frame = 0
                ...
                if self.status == 4 and (time.time() - self.force_update_time_start > self.enforce_202_stop_time) and not self.ready_flag: #3.5
                ...
                if (self.status == 6 and not self.ready_flag):  # 201 지나면서 count 시작
                    ...
                    if self.count_frame == 0:
                        self.force_update_time_start = time.time()
                    self.count_frame += 1
                if self.status == 6 and (time.time() - self.force_update_time_start > self.enforce_202_stop_time) and not self.ready_flag: #3.5
                    self.status -= 1  # 202 status
                    self.status = self.status % 7
                    self.status_detect = True
                    self.count_frame = 0
                    self.force_update_time_start = time.time()
                if self.status == 3 and (time.time() - self.force_update_time_start > self.enforce_102_stop_time) and not self.ready_flag: #3.0
                    self.status -= 1  # 102 status
                    self.status = self.status % 7
                    self.status_detect = True
                    self.count_frame = 0
        ...
        if self.status_detect:
            ...
            self.status_detect_time = time.time()
            #print(self.status_detect_time - self.previous_status_detect_time)

            if self.status is not 0 and self.status_detect_time - self.previous_status_detect_time < self.sequenctial_update_time:
                #print("if loop in",self.status)
                print("status sequential change --> remove update")
                if self.wise == 0:
                    self.status -= 1
                else:
                    self.status += 1
                ...
                #print("safely status change --> update")
                self.previous_status_detect_time = self.status_detect_time
        ...
        cv2.putText(image, "status:" + str(self.status), (220, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.65,
                    (0, 0, 200), 2)
